...
def parse_can_buffer(dat):
  ...
  for j in range(0, len(dat), 0x10):
    ...
    if DEBUG:
      print("  R %x: %s" % (address, codecs.encode(dddat, "hex")))
...
class Panda(object):
  ...
  def can_send_many(self, arr):
    ...
    for addr, _, dat, bus in arr:
      assert len(dat) <= 8
      if DEBUG:
        print("  W %x: %s" % (addr, codecs.encode(dat, "hex")))
      if addr >= 0x800:
  ...
  def kline_drain(self, bus=2):
    ...
    while True:
      ...
      if len(ret) == 0:
        ...
        print("kline drain: "+codecs.encode(ret, "hex"))
      bret += ret
    return bytes(bret)
  ...
  def kline_ll_recv(self, cnt, bus=2):
    ...
    while len(echo) != cnt:
      ...
      if DEBUG and len(ret) > 0:
        print("kline recv: "+codecs.encode(ret, "hex"))
      echo += ret
    return str(echo)
  ...
  def kline_send(self, x, bus=2, checksum=True):
    ...
    for i in range(0, len(x), 0xf):
      ts = x[i:i+0xf]
      if DEBUG:
        print("kline send: "+codecs.encode(ts, "hex"))
