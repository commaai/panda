...
class Panda:
  ...
  def reset(self, enter_bootstub=False, enter_bootloader=False, reconnect=True):
    ...
    try:
      if enter_bootloader:
        self._handle.controlWrite(Panda.REQUEST_IN, 0xd1, 0, 0, b'', timeout=timeout, send_once=True)
      else:
        if enter_bootstub:
          self._handle.controlWrite(Panda.REQUEST_IN, 0xd1, 1, 0, b'', timeout=timeout, send_once=True)
        else:
          self._handle.controlWrite(Panda.REQUEST_IN, 0xd8, 0, 0, b'', timeout=timeout, send_once=True)
    except Exception:
      pass
  ...
  def flash_static(handle, code, mcu_type):
    ...
    try:
      handle.controlWrite(Panda.REQUEST_IN, 0xd8, 0, 0, b'', send_once=True)
    except Exception:
      pass
