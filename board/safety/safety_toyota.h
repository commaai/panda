  ...
  if (((to_send->RDTR >> 4) & 0xF) == 0) {
    ...
    if ((to_send->RIR>>21) == 0x2E4) {
      ...
      if (controls_allowed && actuation_limits) {
        ...
        highest_allowed_torque = min(highest_allowed_torque, max(desired_torque_last - MAX_RATE_DOWN, torque_meas_max + MAX_TORQUE_ERROR));
        lowest_allowed_torque = max(lowest_allowed_torque, min(desired_torque_last + MAX_RATE_DOWN, torque_meas_min - MAX_TORQUE_ERROR));
