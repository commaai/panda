...
const int VOLVO_EUCD_RX_CHECKS_LEN = sizeof(volvo_eucd_rx_checks) / sizeof(volvo_eucd_rx_checks[0]);
...
static void volvo_init(int16_t param) {
  UNUSED(param);
  controls_allowed = 0;
...
static int volvo_c1_rx_hook(CAN_FIFOMailBox_TypeDef *to_push) {
  ...
  bool valid = addr_safety_check(to_push, volvo_c1_rx_checks, VOLVO_C1_RX_CHECKS_LEN,
                                 NULL, NULL, NULL);
...
static int volvo_eucd_rx_hook(CAN_FIFOMailBox_TypeDef *to_push) {
  ...
  bool valid = addr_safety_check(to_push, volvo_eucd_rx_checks, VOLVO_EUCD_RX_CHECKS_LEN,
                                 NULL, NULL, NULL);
...
const safety_hooks volvo_hooks_c1 = {
  .init = volvo_init,
  .rx = volvo_c1_rx_hook,
  .tx = volvo_c1_tx_hook,
  .tx_lin = nooutput_tx_lin_hook,
  .fwd = volvo_c1_fwd_hook,
  .addr_check = volvo_c1_rx_checks,
  .addr_check_len = sizeof(volvo_c1_rx_checks) / sizeof(volvo_c1_rx_checks[0]),
};


const safety_hooks volvo_hooks_eucd = {
  .init = volvo_init,
  .rx = volvo_eucd_rx_hook,
  .tx = volvo_eucd_tx_hook,
  .tx_lin = nooutput_tx_lin_hook,
  .fwd = volvo_eucd_fwd_hook,
  .addr_check = volvo_eucd_rx_checks,
  .addr_check_len = sizeof(volvo_eucd_rx_checks) / sizeof(volvo_eucd_rx_checks[0]),
};
