...
static int hyundai_canfd_tx_hook(CANPacket_t *to_send) {
  ...
  // steering
  const int steer_addr = (hyundai_canfd_hda2 && !hyundai_longitudinal) ? hyundai_canfd_hda2_alt_steer ? 0x110 : 0x50 : 0x12a;
  if (addr == steer_addr) {
...
static int hyundai_canfd_fwd_hook(int bus_num, int addr) {
  ...
  if (bus_num == 2) {
    ...
    hda2_steer_msg = hyundai_canfd_hda2_alt_steering ? 0x110 : 0x50;
    int is_lkas_msg = (((addr == hda2_steer_msg) || ((addr == 0x2a4) && !hyundai_canfd_hda2_alt_steering)) && hyundai_canfd_hda2);
