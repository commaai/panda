...
AddrCheckStruct subaru_gen2_long_addr_checks[] = {
  {.msg = {{ 0x40, 0, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 10000U}, { 0 }, { 0 }}},
  {.msg = {{0x119, 0, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
  {.msg = {{0x13a, 1, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
  {.msg = {{0x13c, 1, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
  {.msg = {{0x240, 1, 8, .check_checksum = true, .max_counter = 15U, .expected_timestep = 50000U}, { 0 }, { 0 }}},
  {.msg = {{0x78f, 2, 8, .check_checksum = false, .max_counter = 0U, .expected_timestep = 50000U}, { 0 }, { 0 }}},
};
#define SUBARU_GEN2_LONG_ADDR_CHECK_LEN (sizeof(subaru_gen2_long_addr_checks) / sizeof(subaru_gen2_long_addr_checks[0]))
addr_checks subaru_gen2_long_rx_checks = {subaru_gen2_long_addr_checks, SUBARU_GEN2_LONG_ADDR_CHECK_LEN};
...
static int subaru_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid) {
    ...
    if ((addr == 0x241) && (bus == alt_bus)) {
      bool cruise_engaged = GET_BIT(to_push, 55U) != 0U;
...
static int subaru_tx_hook(CANPacket_t *to_send) {
  ...
  if (addr == 0x787){
    ...
    // Allow "tester present" and "read data by identifier"
    bool allowed_sid = (sid == 0x3e) || (sid == 0x22);
...
static const addr_checks* subaru_init(uint16_t param) {
  ...
  if (subaru_gen2) {
    if(subaru_longitudinal){
      subaru_rx_checks = (addr_checks){subaru_gen2_long_addr_checks, SUBARU_GEN2_LONG_ADDR_CHECK_LEN};
    }
    else{
