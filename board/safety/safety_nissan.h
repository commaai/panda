...
// wheel speeds
uint16_t nissan_right_front = 0;
uint16_t nissan_left_front = 0;
...
static void nissan_rx_hook(CANPacket_t *to_push) {
  ...
  if (bus == (nissan_alt_eps ? 1 : 0)) {
    ...
    if ((addr == 0x284) || (addr == 0x29a)) {
      nissan_right_front = (GET_BYTE(to_push, 0) << 8) | (GET_BYTE(to_push, 1));
      nissan_left_front = (GET_BYTE(to_push, 2) << 8) | (GET_BYTE(to_push, 3));
    }
    ...
    if (addr == 0x285) {
      ...
      UPDATE_VEHICLE_SPEED((right_rear + left_rear + nissan_right_front + nissan_left_front) / 4.0 * 0.005 / 3.6);
    }
