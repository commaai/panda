...
static int tesla_tx_hook(CANPacket_t *to_send, bool longitudinal_allowed) {
  ...
  if(addr == (tesla_powertrain ? 0x2bf : 0x2b9)) {
    ...
    if (tesla_longitudinal) {
      ...
      violation |= long_accel_checks(raw_accel_max, TESLA_LONG_LIMITS, longitudinal_allowed);
      violation |= long_accel_checks(raw_accel_min, TESLA_LONG_LIMITS, longitudinal_allowed);
    } else {
      violation = true;
    }
  }
