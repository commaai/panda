...
static safety_config volkswagen_meb_init(uint16_t param) {
  ...
  volkswagen_meb_brake_pressure_detected = false;
...
static void volkswagen_meb_rx_hook(const CANPacket_t *to_push) {
  if (GET_BUS(to_push) == 0U) {
    ...
    if (addr == MSG_ESP_05) {
      volkswagen_MEB_brake_pressure_detected = (GET_BYTE(to_push, 3) & 0x4U) >> 2;
    }

    brake_pressed = volkswagen_MEB_brake_pedal_switch || volkswagen_MEB_brake_pressure_detected;
