...
const CanMsg VOLKSWAGEN_MEB_LONG_TX_MSGS[] = {{MSG_MEB_LANE_ASSIST_01, 0, 24}, {MSG_LDW_02, 0, 8}, {MSG_LH_EPS_03, 2, 8}};
...
static int volkswagen_meb_fwd_hook(int bus_num, int addr) {
  ...
  switch (bus_num) {
    ...
    case 2:
      ...
      //if ((addr == MSG_MEB_LANE_ASSIST_01) || (addr == MSG_LDW_02) || (addr == MSG_MEB_ACC_01)) {
