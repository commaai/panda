...
static uint32_t ford_compute_checksum(CANPacket_t *to_push) {
  ...
  if (addr == MSG_BrakeSysFeatures) {
    ...
    chksum += (GET_BYTE(to_push, 2) >> 3) & 0xf;            // VehVActlEng_No_Cnt
    chksum += (GET_BYTE(to_push, 4) >> 5) & 0x3;            // VehVActlEng_D_Qf
...
static int ford_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == FORD_MAIN_BUS)) {
    ...
    if (addr == MSG_EngVehicleSpThrottle2) {
      // Set vehicle state mismatch if speeds from ABS and PCM ECUs are far enough apart.
      // Causes controls allowed to fall
      // Signal: Veh_V_ActlEngz
