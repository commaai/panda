...
const CanMsg VOLKSWAGEN_PQ_STOCK_TX_MSGS[] = {{VW_MSG_HCA_1, 0, 5}, {VW_MSG_LDW_1, 0, 8},
                                              {VW_MSG_GRA_NEU, 0, 4}, {VW_MSG_GRA_NEU, 2, 4}};
const CanMsg VOLKSWAGEN_PQ_LONG_TX_MSGS[] =  {{VW_MSG_HCA_1, 0, 5}, {VW_MSG_LDW_1, 0, 8},
                                              {VW_MSG_ACC_SYSTEM, 0, 8}, {VW_MSG_ACC_GRA_ANZEIGE, 0, 8}};

AddrCheckStruct volkswagen_pq_addr_checks[] = {
  {.msg = {{VW_MSG_LENKHILFE_3, 0, 6, .check_checksum = true, .max_counter = 15U, .expected_timestep = 10000U}, { 0 }, { 0 }}},
  {.msg = {{VW_MSG_BREMSE_1, 0, 8, .check_checksum = false, .max_counter = 0U, .expected_timestep = 10000U}, { 0 }, { 0 }}},
  {.msg = {{VW_MSG_MOTOR_2, 0, 8, .check_checksum = false, .max_counter = 0U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
  {.msg = {{VW_MSG_MOTOR_3, 0, 8, .check_checksum = false, .max_counter = 0U, .expected_timestep = 10000U}, { 0 }, { 0 }}},
  {.msg = {{VW_MSG_MOTOR_5, 0, 8, .check_checksum = true, .max_counter = 0U, .expected_timestep = 20000U}, { 0 }, { 0 }}},
  {.msg = {{VW_MSG_GRA_NEU, 0, 4, .check_checksum = true, .max_counter = 15U, .expected_timestep = 33000U}, { 0 }, { 0 }}},
};
...
static uint32_t volkswagen_pq_get_checksum(CANPacket_t *to_push) {
  ...
  return (uint32_t)GET_BYTE(to_push, (addr == VW_MSG_MOTOR_5) ? 7 : 0);
}
...
static uint8_t volkswagen_pq_get_counter(CANPacket_t *to_push) {
  ...
  if (addr == VW_MSG_LENKHILFE_3) {
    counter = (uint8_t)(GET_BYTE(to_push, 1) & 0xF0U) >> 4;
  } else if (addr == VW_MSG_GRA_NEU) {
...
static uint32_t volkswagen_pq_compute_checksum(CANPacket_t *to_push) {
  ...
  uint8_t checksum = 0U;
  int checksum_byte = (addr == VW_MSG_MOTOR_5) ? 7 : 0;
...
static int volkswagen_pq_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 0U)) {
    ...
    generic_rx_checks((addr == VW_MSG_HCA_1));
  }
  return valid;
}
...
static int volkswagen_pq_tx_hook(CANPacket_t *to_send) {
  ...
  if ((addr == VW_MSG_GRA_NEU) && !controls_allowed) {
    // Signal: GRA_Neu.GRA_Neu_Setzen
    // Signal: GRA_Neu.GRA_Neu_Recall
...
static int volkswagen_pq_fwd_hook(int bus_num, int addr) {
  ...
  switch (bus_num) {
    case 0:
      ...
      break;
    case 2:
      if ((addr == VW_MSG_HCA_1) || (addr == VW_MSG_LDW_1)) {
        // openpilot takes over LKAS steering control and related HUD messages from the camera
        bus_fwd = -1;
      } else if (volkswagen_longitudinal && ((addr == VW_MSG_ACC_SYSTEM) || (addr == VW_MSG_ACC_GRA_ANZEIGE))) {
