const SteeringLimits HYUNDAI_STEERING_LIMITS = {
  .max_steer = 384,             // like stock
  .max_rt_delta = 112,          // max delta torque allowed for real time checks
...
static int hyundai_tx_hook(CANPacket_t *to_send, bool longitudinal_allowed) {
  ...
  if (addr == 832) {
    ...
    if (steering_checks(desired_torque, steer_req, HYUNDAI_STEERING_LIMITS)) {
      tx = 0;
    }
  }
