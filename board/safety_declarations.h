...
typedef struct {
  ...
  const int length[3];               // expected message length, any is acceptable. Array MUST terminate with a zero to know its length.
  ...
  bool valid_length;                 // true if and only if message length is the expected length
