...
bool steer_angle_cmd_checks(int desired_angle, bool steer_control_enabled, const SteeringLimits limits) {
  ...
  if (controls_allowed && steer_control_enabled) {
    ...
    if (limits.enforce_angle_error && vehicle_speed > limits.angle_error_limit_speed) {
      print('lowest', min(max(lowest_desired_angle, meas - ERROR), prev_angle + up))
      print('highest', max(min(highest_desired_angle, meas + ERROR), (prev_angle - down)))  # prev des, meas
