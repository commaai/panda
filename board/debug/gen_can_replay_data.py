...
def build_payload(route, prefix_events):
  ...
  for event_idx, (_, event) in enumerate(events):
    ...
    for address, dat, bus in can_msgs:
      records += struct.pack("<HB8s", address, (bus << 4) | len(dat), dat.ljust(8, b"\x00"))

  payload = bytes(counts + records)
  return payload, len(counts), msg_count, max_count, CP.carFingerprint


def format_array(payload):
  lines = []
...
def main():
  ...
  payload, event_count, msg_count, max_count, fingerprint = build_payload(args.route, args.prefix_events)
  assert len(payload) == event_count + (msg_count * RECORD_SIZE)
...
#define CAN_REPLAY_RECORD_SIZE {RECORD_SIZE}U
#define CAN_REPLAY_DATA_SIZE {len(payload)}U
#define CAN_REPLAY_MAX_MSGS_PER_EVENT {max_count}U

static const uint8_t can_replay_data[CAN_REPLAY_DATA_SIZE] = {{
{format_array(payload)}
}};
"""
