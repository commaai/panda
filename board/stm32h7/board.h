...
void detect_board_type(void) {
  ...
  const uint8_t id1 = detect_with_pull(&tracked_GPIOF, 7, PULL_UP) |
                     (detect_with_pull(&tracked_GPIOF, 8, PULL_UP) << 1U) |
                     (detect_with_pull(&tracked_GPIOF, 9, PULL_UP) << 2U) |
                     (detect_with_pull(&tracked_GPIOF, 10, PULL_UP) << 3U);

  const uint8_t id2 = detect_with_pull(&tracked_GPIOD, 4, PULL_UP) |
                     (detect_with_pull(&tracked_GPIOD, 5, PULL_UP) << 1U) |
                     (detect_with_pull(&tracked_GPIOD, 6, PULL_UP) << 2U) |
                     (detect_with_pull(&tracked_GPIOD, 7, PULL_UP) << 3U);
