...
static const adc_signal_t sbu1_channels[] = {
  ADC_CHANNEL(ADC3, 12),
  ADC_CHANNEL(ADC3, 2),
  ADC_CHANNEL(ADC3, 4),
  ADC_CHANNEL(ADC3, 6),
...
static const adc_signal_t sbu2_channels[] = {
  ADC_CHANNEL(ADC1, 13),
  ADC_CHANNEL(ADC3, 3),
  ADC_CHANNEL(ADC3, 5),
  ADC_CHANNEL(ADC3, 7),
...
static void board_v2_set_harness_orientation(uint8_t orientation) {
  switch (orientation) {
...
static void board_v2_enable_can_transceiver(uint8_t transceiver, bool enabled) {
  switch (transceiver) {
    case 1U:
...
static void board_v2_enable_header_pin(uint8_t pin_num, bool enabled) {
  if (pin_num < 8U) {
...
static void board_v2_set_can_mode(uint8_t mode) {
...
static void board_v2_set_panda_power(bool enable) {
  panda_power = enable;
...
static void board_v2_set_panda_individual_power(uint8_t port_num, bool enable) {
  port_num -= 1U;
  if (port_num < 6U) {
...
static bool board_v2_get_button(void) {
  return get_gpio_input(GPIOG, 15);
}

static void board_v2_set_ignition(bool enabled) {
  ignition = enabled ? 0xFFU : 0U;
  board_v2_set_harness_orientation(harness_orientation);
}

static void board_v2_set_individual_ignition(uint8_t bitmask) {
  ignition = bitmask;
  board_v2_set_harness_orientation(harness_orientation);
}

static float board_v2_get_channel_power(uint8_t channel) {
  float ret = 0.0f;
...
static uint16_t board_v2_get_sbu_mV(uint8_t channel, uint8_t sbu) {
  uint16_t ret = 0U;
  if ((channel >= 1U) && (channel <= 6U)) {
    switch(sbu){
      case SBU1:
...
static void board_v2_init(void) {
  common_init_gpio();
...
static void board_v2_tick(void) {}
