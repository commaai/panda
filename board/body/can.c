...
static void body_can_send_motor_speeds(uint8_t bus, float left_speed_rpm, float right_speed_rpm) {
  CANPacket_t pkt = {0};
  pkt.bus = bus;
...
static void body_can_send_var_values(uint8_t bus, bool ignition, bool enable_motors, uint8_t fault, uint8_t left_z_errcode, uint8_t right_z_errcode) {
  CANPacket_t pkt = {0};
  pkt.bus = bus;
...
static void body_can_send_body_data(uint8_t bus, uint8_t mcu_temp_raw, uint16_t batt_voltage_raw, uint8_t batt_percentage, bool charger_connected) {
  CANPacket_t pkt = {0};
  pkt.bus = bus;
...
static void body_can_process_target(int16_t left_target_deci_rpm, int16_t right_target_deci_rpm) {
