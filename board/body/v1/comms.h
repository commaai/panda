...
void can_rx(void) {
  while ((board.CAN->RF0R & CAN_RF0R_FMP0) != 0) {
    ...
    if (address == (BODY_CAN_ADDR_TORQUE_CMD + board.can_addr_offset)) {
      ...
      int16_t valueL;
      int16_t valueR;
      body_unpack_rpm_targets(dat, &valueL, &valueR);
    ...
    } else if (address == (BODY_CAN_ADDR_SPEED_LIMIT + board.can_addr_offset)) {
      uint8_t dat[5];
      for (int i = 0; i < 5; i++) {
        dat[i] = GET_MAILBOX_BYTE(&board.CAN->sFIFOMailBox[0], i);
      }
      ...
      uint16_t valueL;
      uint16_t valueR;
      body_unpack_speed_limits(dat, &valueL, &valueR);
