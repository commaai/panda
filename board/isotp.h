...
#define isotp_byte_ring_buffer(x, size) \
  static uint8_t elems_##x[size]; \
  static isotp_byte_ring_t x = { .w_ptr = 0U, .r_ptr = 0U, .fifo_size = (size), .elems = elems_##x };

#ifdef STM32H7
__attribute__((section(".axisram"))) isotp_byte_ring_buffer(isotp_tx_q, ISOTP_TX_RING_SIZE)
__attribute__((section(".axisram"))) isotp_byte_ring_buffer(isotp_rx_q, ISOTP_RX_RING_SIZE)
...
#else
isotp_byte_ring_buffer(isotp_tx_q, ISOTP_TX_RING_SIZE)
isotp_byte_ring_buffer(isotp_rx_q, ISOTP_RX_RING_SIZE)
...
static void isotp_drain_bulk_write_buffer(void) {
  while (isotp_write_buffer.len >= 2U) {
    ...
    if (isotp_write_buffer.len < record_len) {
      break;
    }
    if (!isotp_byte_ring_push(&isotp_tx_q, isotp_write_buffer.data, record_len)) {
      break;
    }
...
static bool __attribute__((unused)) isotp_set_tx_arb_id(uint32_t packed_id) {
  ...
  if (!isotp_parse_packed_arb_id(packed_id, &arb_id, &extended)) {
    return false;
  }
...
static uint32_t isotp_decode_stmin_us(uint8_t stmin_raw) {
  if (stmin_raw <= 0x7FU) {
    return ((uint32_t)stmin_raw * 1000U);
  }
  if ((stmin_raw >= 0xF1U) && (stmin_raw <= 0xF9U)) {
    return 1000U;
  }
  return 0U;
}
...
static bool isotp_match_rx_frame(const CANPacket_t *msg) {
  return (msg->fd == 0U) &&
         ...
         (msg->extended == isotp_session.rx_id_extended) &&
...
static void isotp_try_start_next_tx(uint32_t now_us) {
  ...
  if (!isotp_session.configured || (isotp_session.tx.state != ISOTP_TX_IDLE)) {
    return;
  }
  if (can_slots_empty(can_queues[isotp_session.bus]) == 0U) {
    return;
  }

  if (!isotp_pop_next_tx_pdu(isotp_session.tx.buf, &isotp_session.tx.len)) {
    return;
  }
