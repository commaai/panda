...
static void tres_enable_can_transceiver(uint8_t transceiver, bool enabled) {
  ...
  set_gpio_output(&tracked_GPIOG, 11, !(can0_enabled || can2_enabled));
  set_gpio_output(&tracked_GPIOD, 7, !(can0_enabled || can2_enabled));
}
...
static bool tres_read_som_gpio (void) {
  return (get_gpio_input(&tracked_GPIOC, 2) != 0);
}

static void tres_init(void) {
  // Enable USB 3.3V LDO for USB block
  register_set_bits(&tracked_PWR_CR3, PWR_CR3_USBREGEN);
  ...
  set_gpio_mode(&tracked_GPIOC, 12, MODE_OUTPUT);
  ...
  set_gpio_alternate(&tracked_GPIOC, 9, GPIO_AF2_TIM3);
  pwm_init(&tracked_TIM3, 4);
  tres_set_ir_power(0U);

  // Fake siren
  set_gpio_alternate(&tracked_GPIOC, 10, GPIO_AF4_I2C5);
  set_gpio_alternate(&tracked_GPIOC, 11, GPIO_AF4_I2C5);
  register_set_bits(&tracked_GPIOC.OTYPER, GPIO_OTYPER_OT10 | GPIO_OTYPER_OT11); // open drain
