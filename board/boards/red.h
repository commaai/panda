...
static void red_init(void) {
  ...
  set_gpio_pullup(&tracked_GPIOG, 11, PULL_NONE);
  set_gpio_mode(&tracked_GPIOG, 11, MODE_OUTPUT);

  set_gpio_pullup(&tracked_GPIOB, 3, PULL_NONE);
  set_gpio_mode(&tracked_GPIOB, 3, MODE_OUTPUT);

  set_gpio_pullup(&tracked_GPIOD, 7, PULL_NONE);
  set_gpio_mode(&tracked_GPIOD, 7, MODE_OUTPUT);

  set_gpio_pullup(&tracked_GPIOB, 4, PULL_NONE);
  set_gpio_mode(&tracked_GPIOB, 4, MODE_OUTPUT);

  //B1: 5VOUT_S
  set_gpio_pullup(&tracked_GPIOB, 1, PULL_NONE);
  set_gpio_mode(&tracked_GPIOB, 1, MODE_ANALOG);

  // B14: usb load switch, enabled by pull resistor on board, obsolete for red panda
  set_gpio_output_type(&tracked_GPIOB, 14, OUTPUT_TYPE_OPEN_DRAIN);
  set_gpio_pullup(&tracked_GPIOB, 14, PULL_UP);
  set_gpio_mode(&tracked_GPIOB, 14, MODE_OUTPUT);
  set_gpio_output(&tracked_GPIOB, 14, 1);
}

static harness_configuration red_harness_config = {
  .GPIO_SBU1 = &tracked_GPIOC,
  .GPIO_SBU2 = &tracked_GPIOA,
  .GPIO_relay_SBU1 = &tracked_GPIOC,
  .GPIO_relay_SBU2 = &tracked_GPIOC,
  .pin_SBU1 = 4,
  .pin_SBU2 = 1,
  .pin_relay_SBU1 = 10,
  .pin_relay_SBU2 = 11,
