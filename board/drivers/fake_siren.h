...
void siren_tim7_init(void) {
  ...
  register_set(&tracked_TIM7.PSC, 0U, 0xFFFFU);
  register_set(&tracked_TIM7.ARR, 133U, 0xFFFFU);
  register_set(&tracked_TIM7.CR2, (0b10U << TIM_CR2_MMS_Pos), TIM_CR2_MMS_Msk);
  register_set(&tracked_TIM7.CR1, TIM_CR1_ARPE | TIM_CR1_URS, 0x088EU);
  TIM7->SR = 0U;
...
void siren_dac_init(void) {
  ...
  register_set(&tracked_DAC1_CR, DAC_CR_TEN1 | (6U << DAC_CR_TSEL1_Pos) | DAC_CR_DMAEN1, 0xFFFFFFFFU);
...
void siren_dma_init(void) {
  ...
  register_set(&tracked_DMA1_Stream1_PAR, (uint32_t)&(DAC1->DHR8R1), 0xFFFFFFFFU);
  // Setup DMA
  register_set(&tracked_DMA1_Stream1_M0AR, (uint32_t)fake_siren_lut, 0xFFFFFFFFU);
...
void fake_siren_codec_enable(bool enabled) {
  if (enabled) {
    bool success = true;
    success &= i2c_set_reg_bits(&tracked_I2C5, CODEC_I2C_ADDR, 0x2B, (1U << 1)); // Left speaker mix from INA1
    success &= i2c_set_reg_bits(&tracked_I2C5, CODEC_I2C_ADDR, 0x2C, (1U << 1)); // Right speaker mix from INA1
    success &= i2c_set_reg_mask(&tracked_I2C5, CODEC_I2C_ADDR, 0x3D, 0x17, 0b11111); // Left speaker volume
    success &= i2c_set_reg_mask(&tracked_I2C5, CODEC_I2C_ADDR, 0x3E, 0x17, 0b11111); // Right speaker volume
    success &= i2c_set_reg_mask(&tracked_I2C5, CODEC_I2C_ADDR, 0x37, 0b101, 0b111); // INA gain
    success &= i2c_set_reg_bits(&tracked_I2C5, CODEC_I2C_ADDR, 0x4C, (1U << 7)); // Enable INA
    success &= i2c_set_reg_bits(&tracked_I2C5, CODEC_I2C_ADDR, 0x51, (1U << 7)); // Disable global shutdown
    if (!success) {
    ...
    for (uint8_t i=0U; i<10U; i++) {
      if (i2c_clear_reg_bits(&tracked_I2C5, CODEC_I2C_ADDR, 0x4C, (1U << 7))) {
        break;
      }
    }
  }
