...
void gmlan_irq_handler(void);

void setup_timer(void) {
  // register interrupt
  REGISTER_INTERRUPT(GMLAN_BITBANG_IRQ, gmlan_irq_handler, 40000U, FAULT_INTERRUPT_RATE_GMLAN)

  // setup
  register_set(&(GMLAN_BITBANG_TIMER->PSC), (48-1), 0xFFFFU);    // Tick on 1 us
  register_set(&(GMLAN_BITBANG_TIMER->CR1), TIM_CR1_CEN, 0x3FU); // Enable
  register_set(&(GMLAN_BITBANG_TIMER->ARR), (30-1), 0xFFFFU);   // 33.3 kbps

  // in case it's disabled
  NVIC_EnableIRQ(GMLAN_BITBANG_IRQ);
