...
void update_can_health_pkt(uint8_t can_number, uint32_t ir_reg) {
  ...
  if (ir_reg != 0U) {
    ...
    if ((ir_reg & (FDCAN_IR_RF0L)) != 0U) { can_health[can_number].total_rx_lost_cnt += 1U; }
    ...
    if ((FDCANx->TXFQS & FDCAN_TXFQS_TFQF) == 0U) {
      ...
      if (can_pop(can_queues[bus_number], &to_send)) {
        if (can_check_checksum(&to_send)) {
          ...
          for (unsigned int i = 0; i < data_len_w; i++) { BYTE_ARRAY_TO_WORD(fifo->data_word[i], &to_send.data[i*4U]); }
        ...
        } else { can_health[can_number].total_tx_checksum_error_cnt += 1U; }
...
void can_rx(uint8_t can_number) {
  ...
  while ((FDCANx->RXF0S & FDCAN_RXF0S_F0FL) != 0U) {
    ...
    CANPacket_t to_push;
    canfd_fifo *fifo = (canfd_fifo *)(RxFIFO0SA + (rx_fifo_idx * FDCAN_RX_FIFO_0_EL_SIZE));
    ...
    for (unsigned int i = 0; i < data_len_w; i++) { WORD_TO_BYTE_ARRAY(&to_push.data[i*4U], fifo->data_word[i]); }
    can_set_checksum(&to_push);
    int bus_fwd_num = safety_fwd_hook(bus_number, to_push.addr);
    if (bus_fwd_num < 0) { bus_fwd_num = bus_config[can_number].forwarding_bus; }
    if (bus_fwd_num != -1) {
      CANPacket_t to_send;
    ...
    rx_buffer_overflow += can_push(&can_rx_q, &to_push) ? 0U : 1U;
    if (!(bus_config[can_number].canfd_enabled) && (canfd_frame)) { bus_config[can_number].canfd_enabled = true; }
    if (!(bus_config[can_number].brs_enabled) && (brs_frame)) { bus_config[can_number].brs_enabled = true; }
    FDCANx->RXF0A = rx_fifo_idx;
  }
  if ((ir_reg & (FDCAN_IR_PED | FDCAN_IR_PEA | FDCAN_IR_EP | FDCAN_IR_BO | FDCAN_IR_RF0L)) != 0U) { update_can_health_pkt(can_number, ir_reg); }
}
...
uint8_t can_tx_slots_available(FDCAN_GlobalTypeDef *FDCANx) {
  return (uint8_t)((FDCANx->TXFQS & FDCAN_TXFQS_TFFL) >> 16);
}
