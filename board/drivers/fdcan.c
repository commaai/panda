...
// External functions from lluart or other low-level drivers
extern bool llcan_set_speed(FDCAN_GlobalTypeDef *FDCANx, uint32_t speed, uint32_t data_speed, bool non_iso, bool loopback, bool silent);
extern void llcan_clear_send(FDCAN_GlobalTypeDef *FDCANx);
extern bool llcan_init(FDCAN_GlobalTypeDef *FDCANx);
extern int safety_fwd_hook(int bus_num, int addr);
extern int safety_rx_hook(CANPacket_t *to_push);
