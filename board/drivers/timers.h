...
static void timer_init(const tracked_timer *TIM, int psc) {
...
void interrupt_timer_init(void) {
  ...
  register_set(&TRACKED_INTERRUPT_TIMER.PSC, ((uint16_t)(15.25*APB1_TIMER_FREQ)-1U), 0xFFFFU);
  register_set(&TRACKED_INTERRUPT_TIMER.DIER, TIM_DIER_UIE, 0x5F5FU);
  register_set(&TRACKED_INTERRUPT_TIMER.CR1, TIM_CR1_CEN, 0x3FU);
  INTERRUPT_TIMER->SR = 0;
  NVIC_EnableIRQ(INTERRUPT_TIMER_IRQ);
}

void tick_timer_init(void) {
  timer_init(&TRACKED_TICK_TIMER, (uint16_t)((15.25*APB2_TIMER_FREQ)/8U));
