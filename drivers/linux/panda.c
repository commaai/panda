...
const int bus_lookup[] = {0, 1, 2};
const int inf_lookup[] = {0, 1, 2};

struct panda_inf_priv *
panda_get_inf_from_bus_id(struct panda_dev_priv *priv_dev, int bus_id)
{
  if (bus_id < PANDA_NUM_CAN_INTERFACES) {
    int inf_num = inf_lookup[bus_id];
			   ...
			   const struct usb_device_id *id)
{
  ...
  for (inf_num = 0; inf_num < PANDA_NUM_CAN_INTERFACES; inf_num++) {
    ...
    priv_inf->mcu_can_ifnum = bus_lookup[inf_num];
