...
class Panda(object):
        ...
        if device.getVendorID() == 0xbbaa and device.getProductID() == 0xddcc:
          if serial is None or device.getSerialNumber() == serial:
            ...
            if claim:
              ...
              #self._handle.setInterfaceAltSetting(0, 0) #Issue with USB stack
            break
  ...
  def health(self):
    dat = self._handle.controlRead(REQUEST_IN, 0xd2, 0, 0, 13)
  ...
  def enter_bootloader(self):
    try:
      self._handle.controlWrite(PREQUEST_OUT, 0xd1, 0, 0, b'')
  ...
  def get_secret(self):
    return self._handle.controlRead(REQUEST_IN, 0xd0, 1, 0, 0x10)

  # ******************* configuration *******************

  def set_controls_allowed(self, on):
      self._handle.controlWrite(REQUEST_OUT, 0xdc, (0x1337 if on else 0), 0, b'')

  def set_can_baud(self, bus, baud):
    self._handle.controlWrite(REQUEST_OUT, 0xde, bus, 0, struct.pack('I', baud))
    return self.get_can_baud(bus)

  def get_can_baud(self, bus):
    res = self._handle.controlRead(REQUEST_IN, 0xdf, bus, 0, 4)
    if res:
      return struct.unpack("I", res)[0]

  def set_gmlan(self, bus, on):
    self._handle.controlWrite(REQUEST_OUT, 0xdb, bus, bool(on), b'')

  def set_uart_baud(self, uart, rate):
    self._handle.controlWrite(REQUEST_OUT, 0xe1, uart, rate, b'')

  def set_uart_parity(self, uart, parity):
    # parity, 0=off, 1=even, 2=odd
    self._handle.controlWrite(REQUEST_OUT, 0xe2, uart, parity, b'')

  def set_uart_callback(self, uart, install):
    self._handle.controlWrite(REQUEST_OUT, 0xe3, uart, int(install), b'')
  ...
  def serial_read(self, port_number):
    return self._handle.controlRead(REQUEST_IN, 0xe0, port_number, 0, 0x40)
  ...
  def kline_wakeup(self):
    self._handle.controlWrite(REQUEST_OUT, 0xf0, 0, 0, b'')

  def kline_drain(self, bus=2):
    # drain buffer
    bret = bytearray()
    while True:
      ret = self._handle.controlRead(REQUEST_IN, 0xe0, bus, 0, 0x40)
      if len(ret) == 0:
        break
      bret += ret
    return bytes(bret)
  ...
  def kline_ll_recv(self, cnt, bus=2):
    echo = bytearray()
    while len(echo) != cnt:
      echo += self._handle.controlRead(REQUEST_IN, 0xe0, bus, 0, cnt-len(echo))
    return echo
