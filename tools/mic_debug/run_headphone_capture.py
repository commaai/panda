...
REMOTE = '/data/mic-lab/headphone-source-01'
...
def main():
  ...
  metadata = {'source': 'USB AirPods Max', 'sample_specification': sink['sample_specification'],
              'volume': sink['volume'], 'stimulus_peak': 0.1,
        ...
        for key, _ in selector.select(timeout=1):
          ...
          while b'\n' in pending:
            ...
            if line.startswith('{') and json.loads(line).get('recording'):
              ...
              player = subprocess.Popen(['paplay', '--device=' + sink['name'], str(ROOT / 'headphone-tones.wav')])
